...
public class TabScript : MonoBehaviour
{
	...
	void Start()
	{
		...
		//text with time
		timeText = tab.GetComponentsInChildren<Text>()[7];
	...
	private void RecieveInvestmentDisplay(object sender)
	{
		...
		tab1Text.text +="Получена инвестиция от " + investor.Title + " в размере " + investor.Investment + " грн.";
	}
	...
	private void RecieveCreditDisplay(object sender)
	{
		...
		tab1Text.text += "Получен кредит от " + investor.Title + " в размере " + investor.Investment + " грн. на срок " + investor.Period + " мес.";
	}
	...
	private void SetTaxationTypeDisplay(object sender)
	{
		...
		if (t.Taxation_group != 0){
			...
			tab1Text.text +="На предприятии выбрана " + enterprise.Title + " группа налогообложения";
		}
		else {
			Debug.Log("Вы не можете выбрать этот тип налогообложения для вашего предприятия! ");
			tab1Text.text +="Вы не можете выбрать этот тип налогообложения для вашего предприятия! ";
		}
	}
	...
	void Update()
	{
		timeText.text = GameTimer.getDisplayTime();
	}
	...
	public void TabTasks()
	{
		...
		tab5Text.enabled = false;
		tab6Text.enabled = false;
		//remove when appending messages to tab
